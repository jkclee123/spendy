 ...
 */
describe("Stats Navigation Integration", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should update selected month when dropdown option is selected", () => {
    ...
    // Simulate selecting a month from dropdown
  ...
  it("should format category labels with emoji and localized name", () => {
    ...
    const getCategoryLabel = (lang: "en" | "zh-HK") => {
      const name = lang === "zh-HK" ? mockCategoryData.zh_name || mockCategoryData.en_name : mockCategoryData.en_name || mockCategoryData.zh_name;
